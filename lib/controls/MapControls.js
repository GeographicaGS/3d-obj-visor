...
MapControls = function ( object, domElement ) {
	...
	this.panCoef = 100; // Slowdown mouse movement coeficient
	...
	function onMouseDownPan(event) {
		...
		panEnd.set( scope.panX, scope.panY );
		panStart.copy( panEnd );

		scope.doPan = setInterval(doPan, 10);
		
		document.addEventListener( 'mousemove', onMouseMovePan, false );
		document.addEventListener( 'mouseup', onMouseUpPan, false );
	}

	function onMouseMovePan(event) {
		event.preventDefault();
		scope.panX += event.movementX / scope.panCoef;
		scope.panY += event.movementY / scope.panCoef;
	}

	function onMouseUpPan(event) {
		window.clearInterval(scope.doPan);
		scope.panX = 0;
		scope.panY = 0;
		document.removeEventListener( 'mousemove', onMouseMovePan, false );
		document.removeEventListener( 'mouseup', onMouseUpPan, false );
	}
	...
	panControl.id = 'panControl';
	this.domElement.appendChild( panControl );

	// Listen to events events
	...
	panControl.addEventListener( 'mousedown', onMouseDownPan, false );
